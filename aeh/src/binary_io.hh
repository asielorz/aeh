...
namespace aeh
{
	...
	template <typename InputStream, aeh::is_trivially_copyable T> InputStream & read_binary(InputStream & is, T & t) noexcept;
	//! Reads sizeof(T) * n bytes from os into an array pointed to by t.
	template <typename InputStream, aeh::is_trivially_copyable T> InputStream & read_binary(InputStream & is, T t[], int n) noexcept;
	//! Reads sizeof(T) * t.size() bytes from os into an array pointed to by t.
	template <typename InputStream, aeh::is_trivially_copyable T> InputStream & read_binary(InputStream & is, std::span<T> t) noexcept;
	//! Writes sizeof(T) bytes from t into os.
	template <typename OutputStream, aeh::is_trivially_copyable T> OutputStream & write_binary(OutputStream & os, T const & t) noexcept;
	//! Writes sizeof(T) * n bytes from an array pointed to by t into os.
	template <typename OutputStream, aeh::is_trivially_copyable T> OutputStream & write_binary(OutputStream & os, T const t[], int n) noexcept;
	//! Writes sizeof(T) * t.size() bytes from an array pointed to by t into os.
	template <typename OutputStream, aeh::is_trivially_copyable T> OutputStream & write_binary(OutputStream & os, std::span<T const> t) noexcept;
	...
	template <aeh::is_trivially_copyable T> [[nodiscard]] auto checksum(T const & t) noexcept -> uint64_t;
